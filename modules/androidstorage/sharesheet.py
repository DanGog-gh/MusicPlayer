...
class ShareSheet:
    ...
    def share_file_list(self, shared_file_list, app=None):
        try:
            ...
            self.send.putParcelableArrayListExtra(Intent.EXTRA_STREAM, self.parcelable)
            self.send.addFlags(Intent.FLAG_GRANT_READ_URI_PERMISSION)
            if app:
                self.send.setPackage(app)
                mActivity.startActivity(self.send)
        ...
        if api_version > 28:
            ...
            if type(shared_file) == str:
                if exists(shared_file):
                    ...
                    self.MIME_type = SharedStorage().get_file_MIME_type(file_name)
                    cv = ContentValues()
                    cv.put(MediaStoreMediaColumns.DISPLAY_NAME, self.display_name)
                    cv.put(MediaStoreMediaColumns.MIME_TYPE, self.MIME_type)
    ...
    def _legacy_destroy_uri(self, shared_file):
        ...
        cursor = context.getContentResolver().query(
            root_uri, None, self.selection, self.args, None
        )
        uri = None
        if cursor:
